...
import { LayoutDashboard, UserCircle2, FileText, Menu, X } from 'lucide-react'
  ...
  const navLinks = (
    <>
      {navItems.map((item) => {
        ...
        const isMyQuotes = item.href === '/dashboard/quotes'
        const active = isRoot
          ? pathname === '/dashboard'
          : isMyQuotes
              ...
              {navItems.map((item) => {
                ...
                const active = isRoot
                  ...
                  : isMyQuotes
                    ? pathname === '/dashboard/quotes' ||
                      (pathname.startsWith('/dashboard/quotes/') &&
                        !pathname.startsWith('/dashboard/quotes/new'))
                    : pathname === item.href || pathname.startsWith(item.href + '/')
                return (
                  <Link
                    key={item.href}
